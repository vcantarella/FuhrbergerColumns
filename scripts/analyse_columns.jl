...
for i in 1:4
    ...
    if i == 3 || i == 4
        ...
        push!(cins, [c_no3, 0.0, c_so4, c_fe, c_lac_3_4_switch])
        push!(t0s, t_s_34_s) # convert days to seconds
        push!(cins, [c_no3_all_1*1e-3, 0.0, c_so4, c_fe, c_lac_3_4_switch])
        push!(t0s, t_1_s) # convert days to seconds
        push!(cins, [c_no3_all_2*1e-3, 0.0, c_so4, c_fe, c_lac_3_4_switch])
        push!(t0s, t_2_s) # convert days to seconds
        push!(cins, [c_no3_4mM*1e-3, 0.0, c_so4, c_fe, c_lac_3_4_switch])
        push!(t0s, t_4_s) # convert days to seconds
    else
        # For columns 1 and 2, we have a switch in the NO3-
        push!(cins, [c_no3_all_1*1e-3, 0.0, c_so4, c_fe, c_lac])
        push!(t0s, t_1_s) # convert days to seconds
        push!(cins, [c_no3_all_2*1e-3, 0.0, c_so4, c_fe, c_lac])
        push!(t0s, t_2_s) # convert days to seconds
        push!(cins, [c_no3_4mM*1e-3, 0.0, c_so4, c_fe, c_lac])
        push!(t0s, t_4_s) # convert days to seconds
    end
...
function reactive_transport_builder(flow_data::TData, cin_data::CinData, Deff, dx, ϕ, ρₛ, gs)
    ...
    function build_rhs(dynamic_transport, dynamic_c_in, expo_fun, dx, gs,De=De_work, mult_term = (1-ϕ)/ϕ*ρₛ)
        function rhs!(du, u, p, t)
            ...
            fe2_ = @view u[:,4]
            lac_ = @view u[:,5]
            b = @view u[:,6]  # biomass active fraction (immobile))
            b_in = @view u[:,7]  # biomass inactive fraction (immobile)
            ...
            @inbounds for k in 1:n_rows
                ...
                du[k,6] += r_act - r_ina  # biomass active fraction
                du[k,7] += r_ina - r_act  # biomass inactive fraction
            end
        end
    return rhs!
    end
