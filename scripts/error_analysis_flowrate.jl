...
vial_error502stds = 2*vial_error50
vial_weight5095ci = mean_vial_weight50 ± vial_error502stds
mean_vial_weight15 = mean(df15[:, 1])
vial_error15 = std(df15[:, 1])
vial_error152stds = 2*vial_error15
vial_weight15 = mean_vial_weight15 ± vial_error15
vial_weight1595ci = mean_vial_weight15 ± vial_error152stds
...
volumes5095ci = (water_weights50 .+ mean_vial_weight50) .- vial_weight5095ci
volumes15 = (water_weights15 .+ mean_vial_weight15) .- vial_weight15
volumes1595ci = (water_weights15 .+ mean_vial_weight15) .- vial_weight1595ci
...
lines!(ax, Measurements.value.(volumes50), Measurements.uncertainty.(volumes50) ./ Measurements.value.(volumes50) .* 100,
 color=:blue, label="1std deviation Error %")
lines!(ax, Measurements.value.(volumes5095ci), Measurements.uncertainty.(volumes5095ci) ./ Measurements.value.(volumes5095ci) .* 100,
 color=:green, label="95% CI Error %")
...
lines!(ax2, Measurements.value.(volumes15), Measurements.uncertainty.(volumes15) ./ Measurements.value.(volumes15) .* 100,
 color=:blue, label="1std deviation Error %")
lines!(ax2, Measurements.value.(volumes1595ci), Measurements.uncertainty.(volumes1595ci) ./ Measurements.value.(volumes1595ci) .* 100,
 color=:green, label="95% CI Error %")
...
fig
save("plots/error_analysis_flowrate.png", fig)
