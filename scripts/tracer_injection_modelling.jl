...
rhs! = create_tracer_rhs_q_callback(D, c_in, dx)
...
affect!(integrator, event_idx, Q) = integrator.p[3] = Q[event_idx]
...
using DataFrames
dfs = load("data/br_breakthrough_data.jld2")
df1_raw = dfs[string(column)]
df1 = DataFrame(df1_raw)  # Convert ReconstructedDataFrame to DataFrame
println("Columns in df1: ", names(df1))
br = df1[!, :Br]*1e-3
time_exp = df1[!, :time]
fig = Figure()
ax = Axis(fig[1, 1],
...
scatter!(ax, time_exp ./ 3600, br, color = :blue, label = "Experimental Data")
fig
...
function cost(p)
    ...
    valid_idx = findall(x-> x in time_exp, sol.t)
