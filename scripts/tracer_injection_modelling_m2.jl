...
flow_rate_ds = load("data/br_flow_rate_m2.jld2")
for column in [1, 2, 3]
    ...
    start_times = start_times[.!ismissing.(Q_1)]
    end_times = end_times[.!ismissing.(Q_1)]
    Q_1 = Q_1[.!ismissing.(Q_1)]
    Q_1 = Q_1/1e6
    ...
    sol = solve(fastprob, QNDF(linsolve=KLUFactorization()), callback = cb, abstol = 1e-8, reltol = 1e-8,
        maxiters = 10000,
        tstops = end_times,
        )
    br = Br_dic[column]*1e-3
    time = convert.(Float64, time_exp[column])
    # fig = Figure()
    # ax = Axis(fig[1, 1],
    #     xlabel = "Time [hr]",
    #     ylabel = "Br⁻ [mol/L]",
    ...
    function cost(p)
        ...
        sol = solve(prob, QNDF(linsolve= KLUFactorization()), callback = cb, abstol = 1e-8, reltol = 1e-8,
            saveat = time,
            maxiters = 10000,
            tstops = end_times,
            )
        # find the index that match time_exp with sol.t
        result = sol(time; idxs = lastindex(u0))
        return sum(abs2, (br - result.u))
    end
...
large_fig
save("plots/br_breakthrough_fit_m2.png", large_fig)

# Save the parameters
save("data/optimized_tracer_params_m2.jld2", "tracer_params", ps)